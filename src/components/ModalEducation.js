...
import {Modal, Button, Row, Container, Col, DropdownButton, Form} from 'react-bootstrap';
...
function ModalEducation({setPass}) {
    ...
    const saveData=async ()=>{
      console.log(degree,degreeTitle,decDate,board,obtMarks,totalMarks,grade);

      const result= axios
        .post("http://localhost:8000/education_data", {
          degree,degreeTitle,decDate,board,obtMarks,totalMarks,grade
        })
        .then((response) => {
  ...
  return (
    <div>
      ...
      <option value="BS">BS</option>
          ...
          value={degreeTitle} onChange={(e)=>setDegreeTitle(e.target.value)}
        />
            </Col>
          </Row>
  ...
  <input placeholder="Select date" type="date" id="example" class="form-control" value={decDate} onChange={(e)=>setDecDate(e.target.value)} />
  ...
  <i class="fas fa-calendar input-prefix"></i>
        ...
        />
          ...
          aria-describedby="basic-addon1"
          value={obtMarks} onChange={(e)=>setObtMarks(e.target.value)}
        />
      ...
      <option value="C">C</option>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" style={{backgroundColor:'#00897b', color: 'white'}} onClick={saveData}>
            Save Changes
          </Button>
        </Modal.Footer>
      </Modal>
